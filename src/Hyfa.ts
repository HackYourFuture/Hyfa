...
export class Hyfa {
  ...
  async start(): Promise<void> {
    ...
    this.slackClient.onGroupMessageTag(
      this.setupGroupMessageHandler.bind(this)
    );
  ...
  private async setupDirectMessageHandler(
    ...
    console.log(
      `Received direct message from ${message.user}: ${message.text}`
    );
    try {
      let typingIndicatorTS: string | undefined =
        await this.slackClient.sendTypingIndicator(message.channel);
      ...
      setTimeout(() => {
        if (typingIndicatorTS) {
          this.slackClient.deleteTypingIndicator(
            message.channel,
            typingIndicatorTS
          );
      ...
      if (typingIndicatorTS) {
        await this.slackClient.deleteTypingIndicator(
          message.channel,
          typingIndicatorTS
        );
        typingIndicatorTS = undefined;
  ...
  private async setupGroupMessageHandler(
    message: SlackMessageEvent
  ): Promise<void> {
    console.log(
      `Received group message in ${message.channel}: ${message.text}`
    );
  }
}
